...
function Navbar({ sidebarOpen, onToggleSidebar }: {
  ...
  return (
    <div data-tauri-drag-region className="navbar min-h-0 h-11 bg-base-200 border-b border-base-300 px-2 gap-0 shrink-0">
      ...
      {/* data-tauri-drag-region on the non-interactive containers/text makes them
          window drag handles; the button lacks it, so it stays clickable. */}
      <div data-tauri-drag-region className={`navbar-start gap-1.5 min-w-0 ${navigator.platform.includes('Mac') ? 'pl-[72px]' : 'pl-1'}`}>
        <button
          className={`btn btn-ghost btn-xs btn-square ${sidebarOpen ? 'bg-base-300' : ''}`}
        ...
        <div data-tauri-drag-region className="flex items-center gap-1.5 min-w-0 text-sm select-none">
          <span data-tauri-drag-region className="font-semibold text-base-content whitespace-nowrap">Surface Evolver</span>
          {activeFile && (
            <>
              <span data-tauri-drag-region className="text-base-content/25">/</span>
              <span data-tauri-drag-region className="font-mono text-base-content/60 truncate max-w-44">{activeFile}</span>
            </>
          )}
        </div>
      </div>
      ...
      <div data-tauri-drag-region className="navbar-end gap-2">
        ...
        {/* Session badge */}
        {sessionId && (
          <div data-tauri-drag-region className="badge badge-ghost badge-sm font-mono hidden md:flex">
            #{sessionId.slice(0, 8)}
          </div>
        )}
      </div>
...
function StatChip({ label, value }: { label: string; value: string }) {
  return (
    <span data-tauri-drag-region className="flex items-baseline gap-1">
