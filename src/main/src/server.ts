 ...
 *   GET  /health
 *   GET  /api/files
 *   POST /api/files/upload                       ← multipart .fe upload
 ...
 *   POST /api/sessions/:id/scale                 → {scale,energy,area}
 *   GET  /api/sessions/:id/export/dmp            → native SE .dmp file
 *   GET  /api/sessions/:id/export/fe             → raw .fe file
...
function checkAuth(req: Request): boolean {
  ...
  return a.length === b.length && (timingSafeEqual as (a: unknown, b: unknown) => boolean)(a, b);
}
...
async function handleGetMesh(req: Request, sessionId: string): Promise<Response> {
  ...
  try {
    const scalars = new URL(req.url).searchParams.get("scalars") ?? undefined;
    const mesh = await seManager.getMesh(sessionId, scalars);
...
async function handleUploadFile(req: Request): Promise<Response> {
  ...
  if (!ct.includes("multipart/form-data"))
    return detail("Expected multipart/form-data", 400, req);

  let form: FormData;
  try { form = await req.formData(); } catch { return detail("Could not parse form data", 400, req); }

  const file = form.get("file") as File | null;
  if (!file) return detail("Missing 'file' field", 400, req);
  if (!file.name.endsWith(".fe")) return detail("Only .fe files are accepted", 400, req);
  if (file.size > 5 * 1024 * 1024) return detail("File exceeds 5 MB limit", 400, req);

  // Sanitize filename — strip path components, allow alphanumeric/dash/underscore/dot
  const safeName = basename(file.name).replace(/[^a-zA-Z0-9._-]/g, "_");
  const destPath = resolve(join(config.seFeDdir, safeName));
  if (!destPath.startsWith(resolve(config.seFeDdir) + "/"))
    return detail("Invalid filename", 400, req);
  if (existsSync(destPath))
    return detail(`File '${safeName}' already exists — rename and retry`, 409, req);

  const raw = await file.arrayBuffer();
  const buf = new Uint8Array(raw);
  try { writeFileSync(destPath, buf); } catch (e) { return detail(`Write failed: ${e}`, 500, req); }

  return json({
    filename:   safeName,
    size_bytes: buf.length,
    renderable: isRenderable(destPath),
  }, 201, cors);
}

async function handleExportDmp(req: Request, sessionId: string): Promise<Response> {
  const session = sessionStore.get(sessionId);
  if (!session) return detail("Session not found", 404, req);
  if (seManager.isBusy())
    return detail("SE is busy — retry after current operation completes", 409, req);

  let result: { content: string };
  try { result = await seManager.dump(sessionId); }
  catch (e: unknown) {
    const err = e as Error & { isSEError?: boolean };
    if (err.message?.includes("not currently loaded")) return detail(err.message, 409, req);
    return detail(String(e), 500, req);
  ...
  const filename  = `${stem}_${timestamp}.dmp`;

  return json({ filename, content: result.content }, 200, corsHeaders(req));
}

async function handleExportFe(req: Request, sessionId: string): Promise<Response> {
  const session = sessionStore.get(sessionId);
  if (!session) return detail("Session not found", 404, req);

  const fePath = resolve(join(config.seFeDdir, session.fe_file));
  let content: string;
  try { content = readFileSync(fePath, "utf8"); }
  catch { return detail(`Could not read source file: ${session.fe_file}`, 500, req); }

  return json({ filename: basename(session.fe_file), content }, 200, corsHeaders(req));
}

async function handleSetScale(req: Request, sessionId: string): Promise<Response> {
  const session = sessionStore.get(sessionId);
  if (!session) return detail("Session not found", 404, req);
  if (seManager.isBusy())
    return detail("SE is busy — retry after current operation completes", 409, req);

  let body: { scale?: number };
  try { body = await req.json(); } catch { return detail("Invalid JSON body", 400, req); }
  if (typeof body.scale !== "number" || body.scale <= 0)
    return detail("scale must be a positive number", 400, req);

  let result: Awaited<ReturnType<typeof seManager.setScale>>;
  try { result = await seManager.setScale(sessionId, body.scale); }
  catch (e: unknown) {
...
const RE_SCALE      = /^\/api\/sessions\/([^/]+)\/scale$/;
const RE_EXP_DMP    = /^\/api\/sessions\/([^/]+)\/export\/dmp$/;
const RE_EXP_FE     = /^\/api\/sessions\/([^/]+)\/export\/fe$/;
...
async function route(req: Request, path: string, method: string): Promise<Response> {
  ...
  if (method === "POST" && path === "/api/files/upload") return handleUploadFile(req);

  // PUT /api/files/:filename — overwrite existing file in fe/ dir
  {
    const fm = RE_FILE.exec(path);
    if (fm && method === "PUT") {
      const filename = decodeURIComponent(fm[1]);
      if (!filename.endsWith(".fe")) return detail("Only .fe files are accepted", 400, req);
      ...
      try { body = await req.json(); } catch { return detail("Invalid JSON body", 400, req); }
      if (typeof body.content !== "string") return detail("content is required", 400, req);
      try { writeFileSync(destPath, body.content, "utf8"); }
      catch (e) { return detail(`Write failed: ${e}`, 500, req); }
      return json({ filename: safeName, size_bytes: body.content.length }, 200, cors);
    }
  }
  ...
  // ── scale ──
  m = RE_SCALE.exec(path);
  if (m && method === "POST") return handleSetScale(req, m[1]);

  // ── export ──
  m = RE_EXP_DMP.exec(path);
  if (m && method === "GET") return handleExportDmp(req, m[1]);

  m = RE_EXP_FE.exec(path);
  if (m && method === "GET") return handleExportFe(req, m[1]);
